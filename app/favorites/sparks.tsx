...
import { Library, Search, Star } from 'lucide-react-native';
...
export default function FavoriteSparksScreen() {
  const insets = useSafeAreaInsets();
  const { colors } = useThemeStyles();
  const { getSparks } = useAppStore();
  ...
  const favoriteSparks = useMemo<Spark[]>(() => {
    return allSparks.filter((spark) => favoriteSparkIds.includes(spark.id));
  }, [allSparks, favoriteSparkIds]);
  
  const filteredSparks = useMemo(() => {
    return favoriteSparks.filter((spark: Spark) => {
  ...
  }, [favoriteSparks, searchQuery, selectedCategory]);
        ...
        {filteredSparks.length === 0 ? (
          <View style={styles.emptyState}>
            <Text style={[styles.emptyStateText, { color: colors.textSecondary }]}>
              {favoriteSparks.length === 0 
                ? 'No favorite sparks yet. Mark sparks as favorites to see them here.'
