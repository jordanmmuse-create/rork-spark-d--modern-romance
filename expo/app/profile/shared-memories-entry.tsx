...
export default function SharedMemoriesEntryScreen() {
  ...
  const [selectedCategory, setSelectedCategory] = useState<NotebookCategory>('notes');
  const [sendTarget, setSendTarget] = useState<SendTarget>('vault');
  const [attachmentUri, setAttachmentUri] = useState<string | null>(null);
  const [showTitlePrompt, setShowTitlePrompt] = useState(false);
  const inputRef = useRef<TextInput>(null);
  const [pendingSendTarget, setPendingSendTarget] = useState<SendTarget | null>(null);
  ...
  const handleDestinationSelect = (target: SendTarget) => {
    if (Platform.OS !== 'web') {
  ...
  const handleSendEntry = () => {
    ...
    setPendingSendTarget(sendTarget);
  ...
  const handleTitleConfirm = (title: string) => {
    ...
    addJournalEntry({ 
      ...
      sendTarget: pendingSendTarget || sendTarget,
        ...
        >
          ...
          <View style={styles.sendTargetSelector}>
            {sendTargetOptions.map((option) => {
              const isSelected = sendTarget === option.id;
              return (
                <TouchableOpacity
                  key={option.id}
                  style={[
                    ...
                    isSelected && [
                      ...
                      { backgroundColor: '#F97316' + '20', borderColor: '#F97316' },
                    ],
                  ]}
                  onPress={() => handleDestinationSelect(option.id)}
                  activeOpacity={0.7}
                >
                  <Text
                    style={[
          ...
          <TouchableOpacity
            style={[
              styles.sendEntryButton,
              { backgroundColor: (isOverLimit || !journalText.trim()) ? colors.surfaceAlt : '#F97316' },
            ]}
