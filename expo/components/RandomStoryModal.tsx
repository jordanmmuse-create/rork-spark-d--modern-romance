...
interface RandomStoryModalProps {
  visible: boolean;
  onClose: () => void;
  post: InspoPost | null;
  isLiked: boolean;
...
function isValidPost(post: InspoPost | null | undefined): post is InspoPost {
...
export default function RandomStoryModal({
  visible,
  onClose,
  post,
  isLiked,
  ...
  const validPost = isValidPost(post) ? post : null;
  ...
  return (
    ...
    >
      <TouchableOpacity
        ...
        testID="randomizer-modal-overlay"
        activeOpacity={1}
        onPress={handleClose}
      >
...
function RandomPostCard({
  ...
  const safeSectionId = post?.sectionId || 'unknown';
  const safeTags = Array.isArray(post?.tags) ? post.tags : [];
  const safeTitle = post?.title || 'Untitled';
  const safeBody = post?.body || '';
  
  const categoryColors = getCategoryColors(safeSectionId);
  const categoryLabel = getCategoryLabel(safeSectionId);
  const categoryEmoji = getCategoryEmoji(safeSectionId);
  ...
  const resolvedMediaUrl = post.mediaUrl || post.mediaThumbnailUrl || null;
  const resolvedHasMedia = Boolean(post.hasMedia && resolvedMediaUrl);
  ...
  return (
    <FlippableMediaStoryCard
      ...
      storyId={post.id}
      onFlipChange={(state) => {
        console.log('[RandomStoryModal] Card flip state:', state, 'mediaUrl:', resolvedMediaUrl);
      }}
      disabledTouchAreas={
    ...
    >
      <View style={[cardStyles.postCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
        <View style={[cardStyles.categoryBadge, { backgroundColor: categoryColors.background, borderColor: categoryColors.border }]}>
          <Text style={[cardStyles.categoryBadgeText, { color: categoryColors.text }]}>
            {categoryEmoji} {post.isTopStory ? 'TOP STORY' : post.isWeeklyCurated ? 'WEEKLY CURATED IDEAS' : categoryLabel.toUpperCase()}
          </Text>
        </View>
        
        <Text style={[cardStyles.postTitle, { color: colors.text }]}>{safeTitle}</Text>
        <Text style={[cardStyles.postBody, { color: colors.text }]}>{safeBody}</Text>
      ...
      {isSendInspoOpen && (
        <SendContentModal
          ...
          onSend={(message) => {
            console.log('[RandomStoryModal] Send Inspo:', message);
